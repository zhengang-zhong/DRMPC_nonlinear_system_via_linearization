...
class Opt_problem(Stack_model):
    ...
    def define_loss_func(self):
        ...
        W_sample_matrix_temp = np.average(W_sample_matrix,axis=1)
        W_sample_matrix_temp = W_sample_matrix_temp.reshape(Nd,-1,order='F')
        mu_est = np.average(W_sample_matrix_temp, axis = 1).reshape(-1,1)
        ...
        for i in range(N):
            ...
            if i > 0:
                error_sum = 0
                for j in range(i):
                    error_sum += np.linalg.matrix_power(A_cl, i) @ D @ mu_est
                loss += 2 * z_temp.T @ Q @ error_sum
                loss += 2 * v_temp.T @ R @ K @ error_sum
...
class Simulation():
    ...
    def __init__(self, sys_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K, cont_time=True, nonlinear=True, u_0=None,
                 xr=None, ur=None, collect=False, est=False, sin_const=1, N_sample=5, epsilon=1, N_sim=80, data_set=None, N_sample_max=None, N_cmax = None):
