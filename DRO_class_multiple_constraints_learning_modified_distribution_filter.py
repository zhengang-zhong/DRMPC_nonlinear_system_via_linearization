...
class Opt_problem(Stack_model):
    def __init__(self, sys_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K, Lx, Ld, d_hat, r, cont_time=True, nonlinear=True,
        ...
        self.d_hat = d_hat
        self.r = r
        # Eigenvalues, Eigenvectors = np.linalg.eig(self.A_cl)
        # self.Eigenvectors = Eigenvectors


        if collect == False:
            self.N_sample = N_sample
            W_sample_matrix = self.gene_disturbance_online(N_sample, sin_const)
    ...
    def gene_disturbance_online(self, N_sample, sin_const):
    ...
    def CSTR_gaug_discrete(self, x, d):
        '''
        x = [x^T, d^T]^T
        '''
    ...
    def define_loss_func(self):
        ...
        # vi_pre_seq = self.vi_pre_seq

        dt_sys_fn = self.dt_sys_fn
        ...
        x_bar = cp.Variable([Nx, 1])
        u_bar = cp.Variable([Nu, 1])

        x_interm = cp.Variable([Nx_s, 1])
        u_interm = cp.Variable([Nu_s, 1])
        ...
        self.x_interm = x_interm
        self.u_interm = u_interm

        loss = 0
        for i in range(N):
            x_interm_temp = x_interm[i * Nx: (i + 1) * Nx, :]
            u_interm_temp = u_interm[i * Nu: (i + 1) * Nu, :]
            loss += cp.quad_form(x_interm_temp, Q)
            loss += cp.quad_form(u_interm_temp, R)

        x_interm_temp = x_interm[N * Nx: (N + 1) * Nx, :]
        loss += cp.quad_form(x_interm_temp, Q)
        self.loss_func = loss
    def define_constraint(self):
        ...
        x_bar = self.x_bar
        u_bar = self.u_bar
        x_interm = self.x_interm
        u_interm = self.u_interm
        r = self.r
        ...
        d_hat_extend = self.extend_d_hat(d_hat)

        constraint = []

        # Initial state
        constraint += [Z[:Nx,:] == x_init]
        constraint += [x_bar == A @ x_bar + B @ u_bar + delta + d_hat_extend]
        constraint += [r == self.CSTR_gaug_discrete(x_bar, d_hat)]
        constraint += [F @ x_bar <= f]
        constraint += [G @ u_bar <= g]

        # Nominal System evolution

        for i in range(N):
            constraint += [Z[(i+1) * Nx : (i+2) * Nx,:] == A @ Z[i * Nx : (i+1) * Nx,:] + B @ V[i * Nu : (i+1) * Nu, :] + delta + d_hat_extend]
            constraint += [x_interm[i * Nx: (i + 1) * Nx, :] == Z[i * Nx: (i + 1) * Nx, :] - x_bar]
            constraint += [u_interm[i * Nu: (i + 1) * Nu, :] == V[i * Nu: (i + 1) * Nu, :] - u_bar]
        constraint += [x_interm[N * Nx: (N + 1) * Nx, :] ==  Z[N * Nx: (N + 1) * Nx, :] - x_bar ]
    ...
    def extend_d_hat(self, d_hat):
        # return np.vstack([np.zeros([1,1]), d_hat, np.zeros([1,1])])
        return np.vstack([d_hat, np.zeros([2,1])])
    def gene_disturbance(self, N, d, N_sample, sin_const):
        # Generate data: const * sinx

        w_sample = []
        for i in range(N_sample):
            w_temp = sin_const * np.random.uniform(-1., 0.8, N * d)
            # w_temp = sin_const * np.random.uniform(-0.8, 1, N * d)
...
class Simulation():
    ...
    def __init__(self, sys_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K, Lx, Ld, r, cont_time=True, nonlinear=True, u_0=None,
                 xr=None, ur=None, collect=False, est=False,
                 sin_const=1, N_sample=5, epsilon=1, N_sim=80, data_set=None, N_sample_max=None, N_cmax = None):
    ...
    def simulation_gene(self, x_init, N_sim):
        ...
        for i in range(N_sim):
            ...
            wk = sin_const * np.random.uniform(-1, 0.8, Nd).reshape(Nd, 1)
            if nonlinear is False:
                uk = opt_problem.V.value[0:Nu,0].reshape(Nu,1)
            else:
                uk = opt_problem.V.value[0:Nu, 0].reshape(Nu, 1)
                # uk = K @ (xk - xr) + opt_problem.V.value[0:Nu, 0].reshape(Nu, 1)

            y = np.vstack([xk[[0], :], xk[[2], :]])

            x_hat_next = A @ xk_hat + B @ uk + delta + opt_problem.extend_d_hat(dk_hat) + Lx @ (y - self.CSTR_gaug_discrete(xk_hat, dk_hat))
            d_hat_next = dk_hat + Ld @ (y - self.CSTR_gaug_discrete(xk_hat, dk_hat))
            ...
            xk_hat = x_hat_next
            dk_hat = d_hat_next
            if N_cmax is not None and N_c < N_cmax:
                xi_collect += [wk]

                if len(xi_collect) == N:
                    N_c += 1
                    xi_collect_np = np.vstack(xi_collect)
                    W_sample_matrix = np.hstack([W_sample_matrix, xi_collect_np])
                    xi_collect = []
