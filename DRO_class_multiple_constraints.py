    ...
    def simulation_gene(self, x_init, N_sim):
        ...
        for i in range(N_sim):
            opt_problem = Opt_problem(sys_fn, delta_t, N, xk, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K,
                                      cont_time=cont_time, nonlinear=nonlinear, u_0=uk, xr=xr, ur=ur, collect=collect,
                                      est=est,sin_const=sin_const, N_sample=N_sample, epsilon=epsilon,  W_sample_matrix = W_sample_matrix)
            Nu = opt_problem.Nu
