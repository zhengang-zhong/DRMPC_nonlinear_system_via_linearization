import numpy as np
import casadi as ca
from DRO_class_multiple_constraints_learning_modified_distribution_filter import Opt_problem, Simulation
...
if __name__ == "__main__":
    ...
    K =  np.array([[-6.33159126, -4.08697435, -1.1737697, -0.5021129],
          [-0.04402787, -0.03782597, -0.01618324, -0.00892606]]) #Q = ca.diag([10,1,1,1] ) R = ca.diag([1e-1]* Nu)

    Lx = np.array([[ 0.98475743, -0.03074359],
       [ 0.18526631,  0.75600808],
       [ 1.16506026,  1.67394808],
       [ 0.43648359,  1.03137772]])
    Ld = np.array([[ 0.59509589,  0.01123627],[-0.02060668,  0.39146257]])
    r = np.array([[1.2345], [134.15212038]])
    ...
    mass_sim = Simulation(ode, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K, Lx, Ld, r, cont_time=True,
                          nonlinear=True, u_0=u_0,
                          xr=xr, ur=ur, collect=True, est=False, sin_const=sin_const,
                          N_sample=N_sample, epsilon=epsilon, N_sim=N_sim, data_set=None, N_sample_max=None,
                          N_cmax=N_cmax)
    mass_sim.plot_state()