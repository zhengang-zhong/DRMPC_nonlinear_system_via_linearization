...
class Model:
    def __init__(self, sys_fn, sys_linear_fn, policy_fn, delta_t, cont_time=True, nonlinear=False):
        self.sys_fn = sys_fn
        self.sys_linear_fn = sys_linear_fn
        ...
        self.N_nu = sys_linear_fn.sx_in()[1].shape[0]

        self.xk_SX = ca.SX.sym("xk_SX", self.Nx)
        self.uk_SX = ca.SX.sym("uk_SX", self.Nu)
        self.nuk_SX = ca.SX.sym("nuk_SX", self.N_nu)

        # Integrate ODE
        self.dt_sys_fn = self.discretize_sys()
        self.dt_sys_linear_fn = self.discretize_linear_sys()
    ...
    def discretize_linear_sys(self):
        xk_SX = self.xk_SX
        nuk_SX = self.nuk_SX
        sys_fn = self.sys_linear_fn
        delta_t = self.delta_t
        x_next = self.integrator_rk4(sys_fn, xk_SX, nuk_SX, delta_t)
        dt_sys_fn = ca.Function("dt_sys_fn", [xk_SX, nuk_SX], [x_next])
        return dt_sys_fn
...
class Linear_model(Model):
    ...
    def __init__(self, sys_fn, sys_linear_fn, policy_fn, delta_t, cont_time=True, nonlinear=False):
        super().__init__(sys_fn, sys_linear_fn, policy_fn, delta_t, cont_time, nonlinear)

        x0_SX = ca.SX.sym("x0_SX", self.Nx)
        u0_SX = ca.SX.sym("u0_SX", self.Nu)
        nu0_SX = ca.SX.sym("u0_SX", self.N_nu)
        self.x0_SX = x0_SX
        self.u0_SX = u0_SX
        self.nu0_SX = nu0_SX
        ...
        dt_sys = self.dt_sys_linear_fn(xk_SX, nuk_SX)

        A = ca.jacobian(dt_sys, xk_SX)
        B = ca.jacobian(dt_sys, nuk_SX)

        A_fn = ca.Function("A_fn", [xk_SX, nuk_SX], [A])
        B_fn = ca.Function("B_fn", [xk_SX, nuk_SX], [B])
        x_next = A_fn(xk_SX, nuk_SX) @ xk_SX + B_fn(xk_SX, nuk_SX) @ nuk_SX
        self.x_next_lin_fn = ca.Function("x_next_lin_fn", [xk_SX, nuk_SX], [x_next])
        self.delta_fn = ca.Function("delta_fn", [x0_SX, nu0_SX], [np.zeros(xk_SX.shape)])
        self.delta = np.zeros(xk_SX.shape)
        self.A = ca.DM(A)  # Transfer SX to DM
        self.B = ca.DM(B)

class Stack_model(Linear_model):
    def __init__(self, sys_fn, sys_linear_fn, policy_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, K, cont_time=True, nonlinear=False, u_0=None, xr=None,
                 ur=None):
        '''
    ...
    def stack_system(self):
        ...
        Ax = np.zeros([Nx_s, Nx])
        Bx = np.zeros([Nx_s, N_nu_s])
        Dx = np.zeros([Nx_s, Nw_s])
        Du = np.zeros([N_nu_s, Nw_s])
        ...
        for i in range(N):
            mat_temp = B
            for j in range(i + 1):
                Bx[(i + 1) * Nx: (i + 2) * Nx, (i - j) * N_nu: (i - j + 1) * N_nu] = mat_temp  # could be problematic
                mat_temp = A @ mat_temp
        # Dx
        for i in range(N):
            mat_temp = D
        ...
        for i in range(N-1):
            mat_temp = D
            for j in range(i + 1):
                Du[(i + 1) * N_nu: (i + 2) * N_nu, (i - j) * Nd: (i - j + 1) * Nd] = K @ mat_temp
                mat_temp = A_cl @ mat_temp
...
class Opt_problem(Stack_model):
    def __init__(self, sys_fn, sys_linear_fn, policy_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K, cont_time=True, nonlinear=True,
                 u_0=None, xr=None, ur=None, collect=False,  est=False, sin_const=1, N_sample=1, epsilon=1,
                 W_sample_matrix = None):
        super().__init__(sys_fn, sys_linear_fn, policy_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, K, cont_time, nonlinear, u_0, xr, ur)
...
class Simulation():
    ...
    def __init__(self, sys_fn, sys_linear_fn, policy_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K, cont_time=True, nonlinear=True, u_0=None,
    ...
    def simulation_gene(self, x_init, N_sim):
        ...
        sys_fn = self.sys_fn
        sys_linear_fn = self.sys_linear_fn
        ...
        for i in range(N_sim):
            opt_problem = Opt_problem(sys_fn, sys_linear_fn, policy_fn, delta_t, N, xk, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K,
            ...
            vk = opt_problem.V.value[0:Nu,0].reshape(Nu,1)
            uk = policy_fn(xk,vk)
            u_list += uk.full().flatten().tolist()
