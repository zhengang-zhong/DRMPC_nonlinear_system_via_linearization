...
class Opt_problem(Stack_model):
    def __init__(self, sys_fn, delta_t, N, x_init, D, F, f, G, g, F_t, f_t, H, h, Q, Qf, R, K, cont_time=True, nonlinear=True,
                 u_0=None, xr=None, ur=None, collect=False,  est=False, sin_const=1, N_sample=1,
                 epsilon=1,W_sample_matrix = None):
    ...
    def define_constraint(self):
        ...
        F_stack = np.zeros([NF * N, Nx * N])
        for i in range(N):
            F_stack[i * NF : (i+1) * NF, i * Nx : (i+1) * Nx ] = F
        f_stack = np.zeros([NF * N, 1])
        for i in range(N):
            f_stack[i * NF : (i+1) * NF, : ] = f
        ...
        s_list_1D = [cp.Variable() for i in range(N_sk)]
        gamma_list_2D = []
        ...
        for i in range(NF * N):
            ...
            for l in range(N_sk):
                ...
                gamma_list_2D[-1] += [gamma_var]

                xi_k =  W_sample_matrix[:,[l]]
                # TODO: modify this if any terminal constraints  / constraint tightening included
                constraint += [Ii.T @ (F_stack @ Z[Nx : , :] + F_stack @ Dx[Nx : , :] @ xi_k - f_stack) + gamma_var.T @ (h - H @ xi_k) <= s_var]
                constraint += [cp.norm(H.T @ gamma_var - Ii.T @ F_stack @ Dx[Nx : , :], p=np.inf) <= lambda_var]
            constraint += [lambda_var * epsilon + 1/N_sk *  cp.sum(s_list_1D) <= 0]
